package de.frosner.fps.exercises.s7.e7
...
object Par {
  ...
  def parFilter[A](as: List[A])(f: A => Boolean): Par[List[A]] =
    map(parMap[A, (A, Boolean)](as)(a => (a, f(a))))(_.filter(_._2).map(_._1)) // uglyyyyy
...
object Main extends App {
  ...
  println(Par.run(Par.parFilter(List(1, 5, 3))(_ < 5)))
